  ...
  <view class="price-page">
    ...
    <view class="quote-card">
      <view class="quote-table-scroll">
        ...
        <view v-if="showTableLoading" class="table-empty">正在加载报价...</view>
...
import {onHide, onLoad, onPullDownRefresh, onReady, onResize, onShareAppMessage, onShow} from "@dcloudio/uni-app";
...
let eggPriceRequestSeq = 0;

onShareAppMessage(() => {
  return {
    title: `${isToday.value ? '今日' : displayDate.value}${currentArea.value.name}蛋价`,
    path: `/pages/price/price?regionCode=${currentArea.value.code}&date=${valueDate.value}`,
    imageUrl: SHARE_IMAGES.price
  }
})

onLoad((options = {}) => {
  const regionIndex = areaList.value.findIndex(item => item.code === options.regionCode);
  if (regionIndex >= 0) {
    tabIdx.value = regionIndex;
  }
  if (options.date && dayjs(options.date).isValid()) {
    date.value = dayjs(options.date);
  }
})

onShow(() => {
  hideSystemTabBar();
  syncEggPrice({preserveData: Boolean(tableRows.value.length)});
...
onPullDownRefresh(() => {
  syncEggPrice({preserveData: Boolean(tableRows.value.length)}).finally(() => {
...
const showTableLoading = computed(() => {
  return loading.value && !tableRows.value.length;
})
...
function syncEggPrice(options = {}) {
  const {preserveData = false} = options;
  const requestSeq = ++eggPriceRequestSeq;

  if (!preserveData) {
    prices.value = [];
  }

  loading.value = !preserveData || !tableRows.value.length;
  const params = {
  ...
  return fetchEggPriceDetailAndLatest(params)
      .then(({detailPrices, latestPrices}) => {
        if (requestSeq !== eggPriceRequestSeq) {
          return;
        }
      ...
      .catch(() => {
        ...
        if (!preserveData || !prices.value.length) {
          prices.value = [];
        }
      })
      .finally(() => {
...
function startPriceRefreshTimer() {
  ...
  priceRefreshTimer = setInterval(() => {
    syncEggPrice({preserveData: true});
...
function measureQuoteTableHeight() {
  ...
  scopedQuery
      ...
      .exec(rects => {
        ...
        const rows = showTableLoading.value || !tableRows.value.length ? 1 : tableRows.value.length;
